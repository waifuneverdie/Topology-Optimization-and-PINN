...
height = 1;
s = length * height;
ratio_wrt_length = length/nelx;
ratio_wrt_height = height/nely;
...
while change > 0.01 && loop ~=271
  ...
  Mnd = mean(sum(4*x(:) .* (1-x(:)) / (nelx * nely)));
  x = xnew;
  %% PRINT RESULTS
  fprintf(' It.:%5i Obj.:%11.4f, Mnd.:%7.3f, Vol.:%7.3f, ch.:%7.3f\n',loop,c1, Mnd, ...
    mean(sum(xSubs(:)) / (nelx * nely)), change)
%% PLOT DENSITIES
